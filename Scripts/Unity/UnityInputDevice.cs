...
public class UnityInputDevice : InputDevice {
	...
	void Initialize( UnityInputDeviceProfile profile, int joystickId ) {
		...
		for (int i = 0; i < analogMappingCount; i++) {
			...
			var analogControl = AddControl( analogMapping.Target, analogMapping.Handle, analogMapping.Player );
		...
		for (int i = 0; i < buttonMappingCount; i++) {
			...
			AddControl( buttonMapping.Target, buttonMapping.Handle, buttonMapping.Player );
		}
	...
	public override void Update( ulong updateTick, float deltaTime ) {
		...
		for (int i = 0; i < analogMappingCount; i++) {
			var analogMapping = Profile.AnalogMappings[i];
			var targetControl = GetControl( analogMapping.Target, analogMapping.Player );
		...
		for (int i = 0; i < buttonMappingCount; i++) {
			...
			UpdateWithState( buttonMapping.Target, buttonState, updateTick, buttonMapping.Player );
		}
	}
